...
20.事件冒泡和事件捕获，如果父子级别都有相应的事件，那么阻止冒泡或者捕获就可以。如果子级有click事件，同时还要阻止父级的mousedown事件，那么不属于同一种
事件，没办法阻止冒泡或者捕获。
方案1：需要让子级绑定一个mousedown事件，同时阻止冒泡，其他什么也不用做。让父级的mousedown事件什么也不做。
方案2：如果点击的元素不是目标元素，那么就禁止事件发生
